...
show_all_stations() {
  ...
  for i in $(awk "BEGIN { for (i = 1; i <= ${cnt}; i++) { print i } }"); do
    echo "radiko,$(echo "${list}" | xmllint --xpath "concat((/region/stations/station)[${i}]/id/text(), ',', (/region/stations/station)[${i}]/name/text())" - 2> /dev/null)"
  done
  # echo ""

  # ListenRadio
  # echo "Record site type: lisradi"
  # curl --silent "http://listenradio.jp/service/channel.aspx" | jq -r '.Channel[] | "lisradi," + (.ChannelId | tostring) + "," + .ChannelName' 2> /dev/null
  # echo ""
...
if [ -z "${mode}" ]; then
  ...
  echo "Require \"Mode\"" >&2
  exit 1
  type="play"; # defaults to play mode
fi
